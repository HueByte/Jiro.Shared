...
.PARAMETER ConfigPath
    Path to the DocFX configuration file (defaults to dev/docfx.json).
...
param(
	...
	[string]$Action = "Build",
	[string]$ConfigPath = "dev\docfx.json",
	[int]$Port = 8080,
	[switch]$Force,
	[switch]$Verbose
)
...
try {
	Write-ColorOutput "📚 Jiro.Shared DocFX Runner" $InfoColor
	...
	switch ($Action) {
		...
		"Serve" {
			...
			try {
				$structureScript = Join-Path $scriptDir "generate-project-structure.ps1"
				if (Test-Path $structureScript) {
					& $structureScript -OutputPath "dev/docs/project-structure.md"
					Write-ColorOutput "✅ Project structure documentation generated" $SuccessColor
				}
				else {
					Write-ColorOutput "⚠️  Project structure script not found, skipping..." $WarningColor
