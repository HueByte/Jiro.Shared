...
Write-Host "🚀 Jiro.Shared NuGet Deployment Script" -ForegroundColor Cyan
...
$ProjectPath = Join-Path $RepoRoot "source\Jiro.Shared\Jiro.Shared.csproj"
...
try {
    ...
    dotnet build "$RepoRoot\source\Main.sln" --configuration Release
    ...
    # Find the package
    $PackageFile = Get-ChildItem -Path $PackagesDir -Filter "Jiro.Shared.$Version.nupkg" | Select-Object -First 1
    if (-not $PackageFile) {
        throw "Package file not found: Jiro.Shared.$Version.nupkg"
    }
