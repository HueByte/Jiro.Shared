...
if [ -n "$COMMITS_RAW" ]; then
    while IFS='|' read -r hash message author date; do
        if [ -n "$hash" ] && [ -n "$message" ]; then
            FORMATTED_COMMITS="$FORMATTED_COMMITS- **$message** ([\`$hash\`](https://github.com/HueByte/Jiro.Shared/commit/$hash)) by @$author on $date
"
        fi
    done <<< "$COMMITS_RAW"
fi
...
CHANGELOG_PATH="https://huebyte.github.io/Jiro.Shared/docs/changelog/$VERSION.html"
...
**Full Changelog**: https://github.com/HueByte/Jiro.Shared/compare/$LATEST_TAG...$VERSION"
