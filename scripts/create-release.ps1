...
if (-not $SkipFormat) {
    ...
    if (Test-Path "source/Main.sln") {
        try {
            if ($DryRun) {
                Write-ColorOutput "   [DRY RUN] Would run: dotnet format source/Main.sln --verify-no-changes" "Yellow"
            }
            else {
                $formatResult = dotnet format source/Main.sln --verify-no-changes 2>&1
                if ($LASTEXITCODE -ne 0) {
                    Write-ColorOutput "⚠️ Code formatting issues detected. Running format..." "Yellow"
                    dotnet format source/Main.sln
        ...
        Write-ColorOutput "⚠️ No solution file found at source/Main.sln, skipping format check" "Yellow"
    }
}
else {
...
function Get-BuildArtifacts {
    ...
    if (-not [string]::IsNullOrWhiteSpace($CustomPath)) {
        ...
        # Jiro.Shared build output paths for .NET projects
        $searchPaths += @(
    ...
    foreach ($commit in $commitsRaw) {
        if (-not [string]::IsNullOrWhiteSpace($commit)) {
            ...
            if ($parts.Length -eq 4) {
                ...
                $author = $parts[2].Trim()
                $date = $parts[3].Trim()
                $formattedCommits += "- **$message** ([``$hash``](https://github.com/HueByte/Jiro.Shared/commit/$hash)) by @$author on $date"
            }
        }
    }
}
...
$changelogPath = "https://huebyte.github.io/Jiro.Shared/docs/changelog/$Version.html"
...
**Full Changelog**: https://github.com/HueByte/Jiro.Shared/compare/$latestTag...$Version
"@
